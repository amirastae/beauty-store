...
type Undertone = "گرم" | "خنثی" | "سرد";

export default function ShadeLab(){
  const [undertone,setUndertone]=useState<Undertone>("خنثی");
  ...
  const chooseUndertone=(next:Undertone)=>{
    setUndertone(next);
    if(next==="خنثی") return;
    const current=shades.find((item)=>item.id===selected);
    if(current?.tone===next) return;
    const first=shades.find((item)=>item.tone===next);
  ...
  return <section className="shade-lab" id="shade-lab">
    <div className="shade-copy">
      ...
      <div className="undertones" role="group" aria-label="انتخاب آندرتون">
        {(["گرم","خنثی","سرد"] as const).map((item)=><button
          type="button"
          key={item}
          className={undertone===item?"active":""}
          aria-pressed={undertone===item}
          onClick={()=>chooseUndertone(item)}
        >{item}</button>)}
      </div>
      ...
      <div className="shade-options" role="group" aria-label="انتخاب رنگ">
        {options.map((item)=><button
          ...
          title={item.name}
        ><i style={{background:item.hex}}/><span>{item.name}</span></button>)}
      </div>
