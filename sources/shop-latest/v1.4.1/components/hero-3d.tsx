...
export default function Hero3D(){
  ...
  useEffect(()=>{
    ...
    if(reduceMotion || cores<=2){
      const frame=requestAnimationFrame(()=>{ if(alive) setFallback(true) })
    ...
    import('three').then(THREE=>{
      ...
      try{
        ...
        cleanup=()=>{
          ...
          ro.disconnect()
          renderer.dispose()
          scene.traverse((object:Object3D)=>{const mesh=object as Object3D & {geometry?: {dispose:()=>void}};mesh.geometry?.dispose()})
        }
      }catch{
        setFallback(true)
      }
